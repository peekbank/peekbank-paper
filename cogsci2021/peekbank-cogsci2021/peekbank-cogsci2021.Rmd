...
Developing word recognition skills builds a foundation for language development; for example, the speed and accuracy with which infants process words are predictive of later linguistic and general cognitive outcomes [@Marchman2008;@Marchman2018].
...
In aggregating a large database of children's early word recognition, the project pursues two main goals: answering theoretical questions about the development of children's word knowledge and providing data-driven evidence that can inform methodological decisions in word recognition research.
...
All studies tested familiar items, but the database also includes `r sum(summarize_novel_familiar$has_novel)` datasets that tested novel pseudo-words in addition to familiar words. All data, including an up-to-date list of current datasets and corresponding references, are openly available on the Open Science Framework (https://osf.io/pr6wu/).
