...
ggplot(rt_subs, 
       aes(x = trial_order, y = sub_mean_rt, col = dataset_name)) + 
  geom_point(alpha = .1) +
...
Try doing this only to 30 months for data sparsity.



```{r}
rt_model_younger <- rts %>%
  filter(rt_value > 300, age > 12, age < 30) %>%
  mutate(log_rt = log(rt_value), 
         age_centered  = age - 12) %>%
  group_by(english_stimulus_label) 
...
<!--   rename(distractor_label = stimulus_label) -->

<!-- ggplot(by_trial_means, aes(x = age, y = prop_looking)) +  -->
<!--   geom_point(alpha= .2) +  -->
...
<!-- ```{r} -->
<!-- by_trial_means$age_scaled <- scale(by_trial_means$age,  -->
<!--                                    center = TRUE, scale = TRUE) -->

<!-- m1 <- lmer(prop_looking ~ age_scaled  -->
<!--            + (1| administration_id) + (1 | target_label),  -->
<!--      data = by_trial_means) -->
...
<!-- ```{r} -->
<!-- m2 <- lmer(prop_looking ~ age_scaled  -->
<!--            + (1 | administration_id) + (age_scaled | target_label),  -->
<!--      data = by_trial_means) -->
...
<!-- ```{r} -->
<!-- m3 <- lmer(prop_looking ~ age_scaled  -->
<!--            + (1 | administration_id) + (age_scaled | target_label) +  -->
<!--              + (1 | distractor_label),  -->
<!--      data = by_trial_means) -->
...
<!-- ```{r} -->
<!-- m4 <- lmer(prop_looking ~ age_scaled  -->
<!--            + (1 | administration_id) + (age_scaled | target_label) +  -->
