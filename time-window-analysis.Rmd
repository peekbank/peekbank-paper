...
#mout <- foreach(i = 1:5, j = 1:5, .combine='bind_rows') %dopar% { # for testing
mout <- foreach(i = 1:length(startTimes), j = 1:length(windowLengths), .combine='bind_rows') %dopar% {
    # by-trial means
    dft <- df %>% filter(t_norm > startTimes[i], t_norm < (startTimes[i]+windowLengths[j])) %>%
