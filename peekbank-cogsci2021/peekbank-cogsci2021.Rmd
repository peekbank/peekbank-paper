...
We expect to not see any reliability in windows that start and end before target onset, and perhaps not for any windows that end within ~300 ms post-target onset.
It is an open question what space of possible windows will show the greatest reliability.
...
Intriguingly, late end times and long overall window lengths--generally not used by researchers--show perhaps the greatest reliability, suggesting that researchers may consider keeping this data for analysis rather than discarding it.
However, the overall strength of the ICCs are generally much weaker than might be desired, with maximum values of ~0.15 (reached only in 3-year-olds).
Moreover, there is some variation by age group in where the strongest ICCs are found (and in the overall strength of ICCs [GK: add mean +ICC per age?]):
