...
The Peekbank data framework consists of three libraries that help to populate and query a relational database (Fig. \ref{fig.framework_overview}).
...
The schema of the database (Fig. \ref{fig.schema}) is sufficiently general to handle heterogeneous datasets from many studies from many labs, including both manually coded and automated eyetracking data.
...
```{r fig.schema, fig.env = "figure", fig.align = "center", fig.width=3.5, set.cap.width=T, num.cols.cap=1, fig.cap = "Data schema for the peekbank database."}
...
```{r fig.framework_overview, fig.env = "figure", fig.align = "center", fig.width=3.5, set.cap.width=T, num.cols.cap=1, fig.cap = "Overview of the peekbank data ecosystem."}
img <- png::readPNG(here("figures","framework_overview.png"))
grid::grid.raster(img)
```
