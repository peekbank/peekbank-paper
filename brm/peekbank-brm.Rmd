...
<!-- scm: dropped this, because users should read about the schema: In the following sections, we provide details on the database’s organization (or _schema_) and the technical implementation of `peekds`. Users who are primarily interested in using the data can skip these details and focus on access through the `peekbankr` API and the web apps. --> 
...
Typically, timepoints in the `xy_timepoints` table and `aoi_timepoints` table need to be regularized to center each trial's time around the point of disambiguation--such that 0 is the time of target word onset in the trial (i.e., the beginning of _dog_ in "Can you find the _dog_?").
