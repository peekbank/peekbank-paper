...
shorttitle        : "Repository for developmental eye-tracking data"
author: 
...
Another potential challenge is that differences in the design choices and analytic decisions between studies could obscure developmental change.
...
One hurdle to improving the power in infant research is that it can be difficult to develop a priori estimates of effect sizes, and how specific design decisions (e.g., the number of test trials) will impact power and reliability.
...
The `trial_types` table encodes information about each trial \textit{in the design of the experiment},\footnote{We note that the term \textit{trial} is often overloaded, to referring to both a particular combination of stimuli seen by many participants and a participant seeing that particular combination at a particular point in the experiment. We track the former in the `trial_types` table and the latter in the `trials` table.} including the target stimulus and location (left vs. right), the distractor stimulus and location, and the point of disambiguation for that trial.
...
We re-centered timing information to the onset of the target label to facilitate comparison of target label processing across all datasets.^[While information preceding the onset of the target label in some datasets such as co-articulation cues [@Mahr2015] or adjectives [@Fernald2013] can in principle disambiguate the target referent, it has been conventional in the literature to use a standardized point of onset.]
...
Compared to linear interpolation [see e.g., @wass_2014], which interpolates between locations of successive timepoints, constant interpolation has the advantage that it is more conservative, in the sense that it does not introduce new look locations beyond those measured in the original data.
...
To facilitate reproducibility of analyses, we use a versioning system by which successive releases are assigned a name reflecting the year and version, e.g., `2021.1`.
...
A second question is interest is about the variability across items within specific studies. While some studies use many, heterogeneous items, others focus on measuring a much smaller and more homogeneous set. Figure \@ref(fig:items) gives an overview of the variability in accuracy for individual words in each dataset. Although all datasets show a gradual rise in accuracy over chance performance, the number of unique target labels and their associated accuracy vary widely across datasets.
...
Gaze behavior has been at the core of many of the key advances in our understanding of infant cognition [@bergelson2012; @lewwilliams2007; @yurovksy_2017; @fernald1998; @Weisleder2013].
