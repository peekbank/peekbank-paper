...
The `aoi_timepoints` table encode where a child is looking at each point in time, by specifying the coded area of interest (or ``AoI''): looks to the target, looks to the distractor, looks on the screen but away from target and distractor, and missing looks.
All datasets must have this timecourse data, as it where children's looking behavior is actually recorded.
For eyetracking experiments that are automatically rather than manually coded, the`xy_timepoints` table encodes the inferred (x, y) coordinates of fixations on the screen over the course of each trail.
