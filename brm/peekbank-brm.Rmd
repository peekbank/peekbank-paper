...
```{r get_am}
aoi_timepoints <- peekbankr::get_aoi_timepoints(dataset_name = "adams_marchman_2018", 
                                         rle=FALSE)
administrations <- peekbankr::get_administrations(dataset_name = "adams_marchman_2018")
```
...
accs <- aoi_timepoints %>%
  ...
  group_by(t_norm, administration_id, age_group) %>% 
  summarise(correct = mean(aoi == "target") / 
              mean(aoi %in% c("target","distractor"))) %>%
  group_by(t_norm, age_group) %>% 
  summarise(ci = 1.96 * sd(correct) / sqrt(length(correct)), 
            correct = mean(correct))

ggplot(accs, aes(x = t_norm, y = correct, col = age_group)) + 
