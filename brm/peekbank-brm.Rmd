...
The key idea of these methods is that if a child preferentially looks at a target referent (rather than a distractor stimulus) upon hearing a word, this behavior indicates that the child is able to recognize the word and activate its meaning during real-time language processing.
...
We then provide two worked examples of how researchers can use Peekbank. In the first, we reproduce a classic result in the word recognition literature, and in the second we show developmental trends on individual items.
...
An screenshot of the app is shown in Figure \@ref(fig:shiny).
