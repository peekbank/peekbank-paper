...
Studies investigating children's word recognition are typically limited in scope to experiments in individual labs involving small samples tested on a small set of items.
...
One way to overcome this challenge is to combine existing datasets into a large-scale database in order to expand the scope of research questions that can be asked about the the development word recognition abilities. 
This strategy capitalizes on the vast amounts of data that have been collected across labs on infants' word recognition over the past 35 years [@golinkoff2013].
In this paper, we introduce large-scale, open database of infant and toddler eye-tracking data of children's word recognition (*Peekbank*).
The primary goal of the Peekbank database is to facilitate the study of developmental changes in children's word knowledge and recognition speed.
...
One core challenge facing psychology in general, but the study of infant development in particular, are threats to the replicability and infant work, given that many studies in infant research are underpowered []
**TO DO: ALIGN CHALLENGES WITH tidybits/ use cases**
- computational reproducibility and teaching

## Understanding the development of children's word knowledge on the level of items
- item-level analyses
...
As illustrated in Figure \@ref(fig:fig-framework-overview), the Peekbank framework consists of four main components: (1) a set of tools to convert eye-tracking datasets into a unified format; (2) a relational database populated with data in this unified format and (3) a set of tools to retrieve data from this database (4) a web app (using the Shiny framework) for visualizing the data.
<!-- scm: the graph shows visualizations / shiny as a 4th top-level component --> 
These components are supported by three libraries.
The `peekds` library (for the R language; [@R-base] helps researchers convert existing datasets to use the standardized format of the database.
...
The `peekbankr` library (R) provides an application programming interface, or API, that offers high-level abstractions for accessing the tabular data stored in Peekbank.
...
To normalize across trials and across experiments, all timecourses are computed so that the time of 0 ms represents the onset of disambiguating material (i.e., the beginning of _dog_ in "can you find the _dog_?").
...
```{r fig-schema, fig.align = "center", out.height="4.5in", set.cap.width=T, num.cols.cap=1, fig.cap = "The Peekbank schema. Each square represents a table in the relational database."}
...
The database currently includes 11 looking-while-listening datasets comprising *N*=`r sum(summarize_datasets$num_subj)` total participants (Table \@ref(tab:dataset-table)).
...
All data (and accompanying references) are openly available on the Open Science Framework (osf.io/pr6wu).
