...
authornote: |
  **Acknowledgements.** We would like to thank the labs and researchers that have made their data publicly available in the database. For further information about contributions, see https://langcog.github.io/peekbank-website/docs/contributors/.
...
```{r fig-schema, fig.env = "figure", fig.align = "center", out.height="4.5in", set.cap.width=T, num.cols.cap=1, fig.cap = "The Peekbank schema. Each darker rectangle represents a table in the relational database."}
...
For participant age, we include the fields `lab_age` and `lab_age_units` to record how the original lab encoded age, as well as an additional field, `age`, to encode age in a standardized format across datasets, using months as the common unit of measurement.
...
They span a wide age spectrum with participants ranging from `r min(dataset_info$age,na.rm=TRUE)` to `r max(dataset_info$age,na.rm=TRUE)` months of age, and are balanced in terms of gender (`r round(mean(dataset_unique_subj$sex=="female"),3)*100`% female; `r round(mean(dataset_unique_subj$sex=="male"),3)*100`% male; `r round(mean(dataset_unique_subj$sex=="unspecified"),3)*100`% unreported). 
